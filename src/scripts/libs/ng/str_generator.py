...
def sha1_hexdigest(input_string, size=20):
    """Generate hexdigest(lower case) for the input string.
    size specifies the number of hex digits in the digest,
    which is 40 in maximum."""
    sha1 = hashlib.sha1()
...
def unique_id(size=40):
    """Generate unique ID(as upper case hex string). size specifies
     the number of characters in the ID, which is 40 in maximum."""
